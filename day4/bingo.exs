...
defmodule Bingo do
  ...
  def mark_boards(boards_raw, numbers) do
    ...
    {_, {board_index, last_number}} = Enum.reduce_while(numbers, {boards, nil}, fn num, {updated_boards, _} ->
      # loop through each board
      {marked_boards, index} = Enum.reduce_while(updated_boards, {[], nil}, fn {rows, index}, {updated_boards, _} ->
        # loop through each row
        {marked_rows, i} = Enum.reduce_while(rows, {[], nil}, fn row, {marked, _} ->
          # remove number from row
          marked_row = Enum.reject(row, &(&1 == num))
          # if remains empty row return board index
          if marked_row == [], do: {:halt, {:break, index}}, else: {:cont, {marked ++ [marked_row], nil}}
        end)
        if i, do: {:halt, {:break, i}}, else: {:cont, {updated_boards ++ [{marked_rows, index}], nil}}
      end)
      if index, do: {:halt, {:break, {index, num}}}, else: {:cont, {marked_boards, nil}}
    end)
    IO.puts "Board: #{board_index}, last number: #{last_number}"
  ...
  defp get_final_score(boards_raw, numbers, board_index, last_number) do
    ...
    score = Enum.reduce(rows, 0, fn row, acc_total ->
      s = row
