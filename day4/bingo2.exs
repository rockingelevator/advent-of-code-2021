...
defmodule Bingo do
  ...
  defp mark([num | tail], {previous_win_index, previous_win_number}, boards) do
    {updated_boards, b_index, b_number} = Enum.reduce(boards, {[], previous_win_index, previous_win_number}, fn {rows, i}, {marked_rows, b_i, b_n} ->
           {bingo_index, bingo_number, updated_rows} = Enum.reduce_while(rows, {nil, nil, []}, fn row, {_, _, marked_rows} ->
